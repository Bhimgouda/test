...
const MANIFEST = require("./localvariables.json");

// User's (private key/secret phrase/seed phrase/mnemonic) to sign the interaction
const MNEMONIC =
  "above cupboard brown mimic bargain tomato lecture want pool prefer busy flush";
...
const deployTodoListLogic = async () => {
  ...
  const todoListLogic = new LogicFactory(MANIFEST, wallet);

  // Submitting the Interaction to the network to deploy the logic
  const ixResponse = await todoListLogic.deploy("");
